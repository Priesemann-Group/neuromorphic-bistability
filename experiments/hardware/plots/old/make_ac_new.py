...
def coefficients(indices, conf):
    freq = conf.freqs[indices[1]]
    seed = conf.seeds[indices[2]]
    spikes = np.load(conf.paths[indices[0]].format(freq, seed))
...
if __name__ == "__main__":
    ...
    shape = (conf.Npaths, conf.Nfreqs, conf.Nseeds)
    ...
    np.savez("data/ac.npz", rks=rks, taus=taus)
