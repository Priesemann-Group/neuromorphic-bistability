...
if __name__ == "__main__":
    ...
    h = data["h"] * 1e-3
    h_selection = data["h_selection"] * 1e-3
    bins = data["bins"][:-1]
    bins -= bins[0]
    mask = np.logical_and(bins >= 16.0, bins < 19.0)
    bins = bins[mask] - 16.0
    ...
    ax2.fill_between(bins, np.zeros_like(prediction_high), prediction_high * 250,
    ...
    ax0.fill_between(bins, np.zeros_like(prediction_low), prediction_low * 250,
    ...
    fax.yaxis.set_label_coords(-0.25, 0.5)
